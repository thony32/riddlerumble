...
const Party = ({ params }: { params: { id: string } }) => {
    ...
    const Completionist = ({ params }: { params: { id: string } }) => {
        useEffect(() => {
        ...
        useEffect(() => {
            if (isOpen && !disableRoomFun.isPending && !disableRoomFun.isSuccess) {
                disableRoomFun.mutate()
            }
        }, [isOpen, disableRoomFun])
    ...
    const timerRender: CountdownRendererFn = ({ minutes, seconds, completed }) => {
        if (completed) {
            return <Completionist params={params} />
        } else {
